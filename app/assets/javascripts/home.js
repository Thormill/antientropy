// animation for main page // work with "menu-item-id" property
colors = ['blue', 'purple', 'green', 'orange', 'yellow', 'pink'];

$(document).ready (function () { 
  clear_all_opacity();
})

function clear_all_opacity() {
  var i = 1;
  do {
    $('[color-item-id=' + i + ']').css('opacity', '0.1');
    clear_border(i);
    i += 1;
  } while ( $('div').is('[color-item-id=' + i + ']') )  
}

function clear_border(colorItemId) {
  for (var i = 0; i < colors.length; i++)
    $('[color-item-id=' + colorItemId + ']').removeClass(colors[i]);
}

function menu_click(marginTop, colorItemId) {
  console.log($('[color-item-id=' + colorItemId + ']').position().top);
  clear_all_opacity();

  $('#content').animate({
    marginTop: marginTop
  })

  $('[color-item-id=' + colorItemId + ']').css('opacity', '1');
  $('[color-item-id=' + colorItemId + ']').addClass(colors[Math.floor(Math.random()*6)]);
}

function menu_item_click(menuItemId) {
  $('[menu-item-id=' + menuItemId + ']').click()
}
